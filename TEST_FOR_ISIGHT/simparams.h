...
static constexpr int NCOND = 3, NINEQ = 3 * NDIV;//THE QUANTITY OF CONDITIONS AND INEQUALITIES
...
static vector<dbl> BETA(NDIV), OMG_ETA(NDIV), DIST(NDIV), ALPHA(NDIV);//線形補間用
...
static RitzMethod Bt(vbase, length_LL), forOMGET(vbase, length_LL);
